...
public partial class SerializedObjectTest
{
	...
	public void Dictionary_Value()
	{
		...
		// the writes through the sub-object should have changed the underlying dictionary value
		Assert.AreEqual( new Vector3( 32, 33, 34 ), source[1] );
		Assert.AreEqual( new Vector3( 32, 33, 34 ), obj.First().GetValue<Vector3>() );
		Assert.AreEqual( Vector3.Down, source[3] );
	}
}
