...
public partial class SerializedObjectTest
{
	[TestMethod]
	...
	public void HashSet_Value()
	{
		...
		var firstProperty = obj.First();
		var originalValue = firstProperty.GetValue<Vector3>();
		Assert.IsTrue( firstProperty.TryGetAsObject( out var vectorObject ) );
		...
		// Known limitation: writes through a TryGetAsObject sub-object are LOST for HashSet
		// elements - there is no in-place replacement path for set entries, so the set is
		// unchanged. Element replacement works via SetValue on the element property instead
		// (see HashSet_ModifyElement). Pin the current behavior so a change is deliberate.
		Assert.IsTrue( source.Contains( originalValue ) );
		Assert.IsFalse( source.Contains( new Vector3( 32, 33, 34 ) ) );
		Assert.AreEqual( 2, obj.Count() );
	}
