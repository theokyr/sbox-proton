...
public static class JsonTestUtils
{
	...
	internal static void RunRoundTripTest( JsonObject source, JsonObject target, string testName, HashSet<Json.TrackedObjectDefinition> definitions )
	{
		...
		// Compare the raw nodes - the null-stripped clones above are only for readable
		// output. Comparing stripped copies would hide a patch that materializes an
		// explicit null where the target omits the property entirely.
		bool areEqual = JsonNode.DeepEquals( result, target );
