...
[TestClass]
public class PrefabInstanceDiffTest
{
	[TestMethod]
	[DataRow( 1, 10 )]
	[DataRow( 5, 20 )]
		...
		int prefabCount, int mutations )
	{
		...
		try
		{
			...
			for ( int i = 0; i < prefabCount; i++ )
			{
				...
				prefabDisposables.Add( SceneTests.Helpers.RegisterPrefabFromJson( resourceName, prefabJson.ToJsonString() ) );
	...
	int prefabCount, int mutations )
	{
		...
		try
		{
			...
			for ( int i = 0; i < prefabCount; i++ )
			{
				...
				prefabDisposables.Add( SceneTests.Helpers.RegisterPrefabFromJson( resourceName, combinedPrefabJson.ToJsonString() ) );
