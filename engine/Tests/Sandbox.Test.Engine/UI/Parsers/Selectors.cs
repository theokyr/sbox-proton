...
public class SelectorsTest
{
	...
	[TestMethod]
	public void HasSelectorClassesAreLowercased()
	{
		// The parser lowercases class names, both on the outer selector and inside :has()
		StyleBlock entry = new();
		Assert.IsTrue( entry.SetSelector( ".Parent:has(.Child)" ) );
		Assert.AreEqual( "parent", entry.Selectors[0].Classes.First() ); // lowercased
		Assert.IsNotNull( entry.Selectors[0].Has );
		Assert.AreEqual( "child", entry.Selectors[0].Has[0].Classes.First() ); // lowercased
	}
