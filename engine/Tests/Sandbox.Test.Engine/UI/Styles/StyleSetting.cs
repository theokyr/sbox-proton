...
public class StyleSettingTest
{
	...
	public void SetBorderImage()
	{
		...
		// The url() stays a lazy texture - only the parsed keywords and slice are asserted here.
		Assert.AreEqual( BorderImageRepeat.Stretch, styles.BorderImageRepeat );
		Assert.AreEqual( BorderImageFill.Filled, styles.BorderImageFill );

		// A single slice value applies to all four sides.
		Assert.IsTrue( styles.BorderImageWidthTop.HasValue );
		Assert.AreEqual( 50, styles.BorderImageWidthTop.Value.Value );
		Assert.AreEqual( 50, styles.BorderImageWidthRight.Value.Value );
		Assert.AreEqual( 50, styles.BorderImageWidthBottom.Value.Value );
		Assert.AreEqual( 50, styles.BorderImageWidthLeft.Value.Value );
	}
