...
public class LeaderboardTest
{
	...
	public async Task Country( string countrycode )
	{
		...
		// An empty filtered response would make the per-row asserts below vacuous
		Assert.IsTrue( board.Entries.Length > 0, $"Expected at least one entry for country '{countrycode}'" );
	...
	public async Task Ascending()
	{
		...
		Assert.IsTrue( board.Entries.Length > 0, "Expected at least one entry" );
		...
		// Entries should actually be sorted ascending by value
		for ( int i = 1; i < board.Entries.Length; i++ )
		{
			Assert.IsTrue( board.Entries[i].Value >= board.Entries[i - 1].Value, $"Entry {i} value {board.Entries[i].Value} is lower than previous value {board.Entries[i - 1].Value}" );
		}
