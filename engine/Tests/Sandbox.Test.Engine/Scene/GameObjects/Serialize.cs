...
[TestClass]
[DoNotParallelize]
public class SerializeTest : SceneTest
{
	...
	public void CloneWithReferences()
	{
		...
		// The copies must get fresh ids, not share them with the source components
		Assert.AreNotEqual( go1comp1.Id, go2comp1.Id );
		Assert.AreNotEqual( go1comp2.Id, go2comp2.Id );
		Assert.AreNotEqual( go1comp3.Id, go2comp3.Id );
	...
	public void CloneRquiredComponentProperty()
	{
		...
		// The reference must point at the clone's own required component, not the original's
		Assert.IsNotNull( cloneComp.RequiredComponent );
		Assert.AreNotSame( originalComp.RequiredComponent, cloneComp.RequiredComponent );
		Assert.AreSame( clone.GetComponent<ComponentIdTest>(), cloneComp.RequiredComponent );
	}
	...
	public void SerializeWithChildren()
	{
		...
		for ( int i = 0; i < childrenCount; i++ )
		{
			...
			child.Components.Create<ComponentIdTest>();
		}
	...
	public void DeserializeNullFields()
	{
		...
		// The deserialized component's field should be null, not the initializer value
		Assert.IsNotNull( go2comp1 );
		Assert.AreEqual( null, go2comp1.ReferenceTypeWithIntializer );
	}
	...
	public void DeserializeNullFieldsIntoExistingObject()
	{
		...
		var go2comp2 = go2.Components.Create<ComponentWithInitializer>();
		...
		// Known divergence from the fresh-component path (DeserializeNullFields above):
		// deserializing into an EXISTING component does NOT apply the explicit null -
		// the old value is kept. Pinned so a behavior change here is deliberate.
		Assert.AreEqual( ComponentWithInitializer.TestReferenceType, go2comp2.ReferenceTypeWithIntializer );
	}
	...
	public void SerializesComponentEnabledFlag()
	{
		...
		var enabledComponent = go1.Components.Create<ComponentIdTest>( true );
		var disabledComponent = go1.Components.Create<ComponentIdTest>( false );
	...
	public void ClearAbsentFieldsOnDeserializeOptionsDeserializeClearsPropertiesAbsentFromJson()
	{
		...
		// Serialize a component with a null value using SkipNulls â€” the key is omitted from JSON.
		var go = new GameObject();
		var comp = go.Components.Create<ComponentWithInitializer>();
		comp.ReferenceTypeWithIntializer = null;
