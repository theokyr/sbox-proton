...
[TestClass]
[DoNotParallelize]
public class CloneTest : SceneTest
{
	...
	public void ComponentNotClonedFlag()
	{
		...
		var a = scene.CreateObject();
		var comp = a.AddComponent<ComponentWithOnStartOnAwake>();
		comp.Flags |= ComponentFlags.NotCloned;

		var clone = a.Clone();

		Assert.IsNull( clone.GetComponent<ComponentWithOnStartOnAwake>() );
	}
	...
	public void CloneSelfReferencingPrefabSceneProperty( bool breakBeforeClone )
	{
		...
		try
		{
			...
			// We also want to test cloning broken-from-prefab objects
			if ( breakBeforeClone )
			{
			...
			var prefabInstance = gameObjectToClone.Clone();
