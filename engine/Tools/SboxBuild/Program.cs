...
internal class Program
{
	static int Main( string[] args )
	{
		...
		AddBuildProtonCommand( rootCommand );
	...
	private static void AddBuildProtonCommand( RootCommand rootCommand )
	{
		var cmd = new Command( "build-proton", "Build a Windows apphost build suitable for running through Proton" );
		...
		cmd.AddOption( targetPlatformOption );
		cmd.AddOption( runtimeOption );
		cmd.AddOption( cleanOption );
		cmd.AddOption( skipArtifactsOption );

		cmd.SetHandler( ( string targetPlatform, string runtime, bool clean, bool skipArtifacts ) =>
		{
			Environment.ExitCode = (int)BuildProton.Run( targetPlatform, runtime, clean, skipArtifacts );
